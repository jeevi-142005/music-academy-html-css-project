# 🎵 Music Academy Website

This is a simple and responsive **Music Academy website** developed using **HTML and CSS**.  
The project is designed to showcase a music academy's information and allow users to explore courses and register easily.

## 📌 Pages Included

- 🏠 **Home Page** – Introduction to the Music Academy  
- 📝 **Registration Page** – Student registration form  
- ℹ️ **About Us Page** – Information about the academy  
- 📋 **Details Page** – Courses, timings, and contact details  

## 🛠️ Technologies Used

- HTML 
- CSS 

## 🎯 Features

- Clean and user-friendly UI  
- Multi-page navigation  
- Simple and responsive layout  
- Beginner-friendly design
  
## 🚀 How to Run the Project

1. Download or clone this repository  
2. Open home.html in any web browser  
3. Navigate through the pages   

## 📸 Project Preview

<img width="1920" height="1080" alt="Screenshot (114)" src="https://github.com/user-attachments/assets/c41a9eb9-c4f7-4e82-9ae8-e0074dba7d87" />
<img width="1920" height="1080" alt="Screenshot (115)" src="https://github.com/user-attachments/assets/052dd556-904e-4cbb-b1a1-8d4fef82ff52" />
<img width="1920" height="1080" alt="Screenshot (118)" src="https://github.com/user-attachments/assets/5e54d3d5-8ddc-47a2-83c1-8186dc114c76" />
<img width="1920" height="1080" alt="Screenshot (117)" src="https://github.com/user-attachments/assets/6ea6953a-5add-4750-baa4-da600ba1e5b4" />


## 👩‍💻 Developed By

**Jeevika R**  
Student 

⭐ This project was created as part of my learning journey in Web Development.

